...
struct file_new {
	char name;
	char context;
	struct file_new *nextdata;
};
struct file_new *glbptr = NULL;
struct file_new *nowptr = NULL;
...
const hcmd_entry cmd_data[CMD_COUNT] = {
	...
	[CMD_LS] = {.cmd = "ls", .func = show_ls, .description = "List xxd storage."},
	[CMD_CAT] = {.cmd = "cat", .func = show_cat, .description = "Catch the context of the list"}
};
...
void insert_datachain(struct file_new **first, char *name, char *context)
{
	struct file_new *checkptr;
	//struct file_new *prevptr;
	struct file_new *newptr;
	struct file_new *firstptr;
	
	write(fdout, name, strlen(name)+1);
	write(fdout, "\r\n", 3);
	write(fdout, context, strlen(context)+1);
	write(fdout, "\r\n", 3);
	...
	while(1){
		if(*name != checkptr->name){
			checkptr = checkptr->nextdata;
		}else{
			write(fdout, name, strlen(name)+1);
			write(fdout, " is existed.", 13);
			write(fdout, "\r\n", 3);
			return;
		}
		if(checkptr->nextdata == NULL);
		break;
	}
	
	newptr = malloc(sizeof(struct file_new));//create new memory
	newptr->name = *name;
	newptr->context = *context;
	newptr->nextdata = NULL;

	if(nowptr == NULL){
		firstptr->name = newptr->name;
		firstptr->context = newptr->context;
		nowptr = firstptr;
	}else{
		nowptr = newptr;
	}
...
void show_xxd(int argc, char *argv[])
{
    ...
    int size;
    int i;
	struct file_new *dataptr = NULL;
	dataptr->nextdata = NULL;
	
	/*can not find the files with xxd*/
	...
	insert_datachain( &dataptr, argv[1], &ch);
...
void show_ls(int argc, char *argv[])
{
	struct file_new *searchptr;
	
	searchptr = glbptr;
	
	write(fdout, "Here is list.", 14);
	write(fdout, "\r\n", 3);
	
	while( searchptr->nextdata != NULL)
	{
		write(fdout, &searchptr->name, sizeof(searchptr->name) + 1);
		write(fdout, "\r\n", 3);
		searchptr = searchptr->nextdata;
	}
	return;
}
...
void show_cat(int argc, char *argv[])
{
	struct file_new *catchptr;
	
	catchptr = glbptr;
	
	write(fdout, "Here is catch.", 15);
	write(fdout, "\r\n", 3);
	
	while(1)
	{
		if(catchptr->nextdata != NULL){
			if(catchptr->name == *argv[1]){
				write(fdout, &catchptr->name, sizeof(catchptr->name) + 1);
				write(fdout, &catchptr->context, sizeof(catchptr->context) + 1);
				write(fdout, "\r\n", 3);
				break;
			}else{
				catchptr = catchptr->nextdata;
			}
		}else{
			write(fdout, "No result.", 11);
			write(fdout, "\r\n", 3);
			break;
		}
	}
